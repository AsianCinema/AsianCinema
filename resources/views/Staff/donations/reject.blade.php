...
@section('breadcrumb')
    ...
    <li class="active">
        <a href="{{ route('staff_donation_reject_form', ['id' => $donation->id]) }}" itemprop="url"
           class="l-breadcrumb-item-link">
            <span itemprop="title" class="l-breadcrumb-item-link-title">Reject Donation</span>
        </a>
    </li>
@endsection
...
@section('content')
    <div class="container box">
        <h2>Reject: {{ $donation->transaction }}</h2>
        <div class="table-responsive">
            <form role="form" method="POST"
                  action="{{ route('staff_donation_reject',['id' => $donation->id]) }}">
                @csrf
                <div class="table-responsive">
